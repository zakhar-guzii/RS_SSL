...
- **Recording** — the ~15-second capture of raw, timestamped total-acceleration samples (x, y, z) triggered by the button press, sent as-is to the Backend. The App does no preprocessing. Recordings are ephemeral: the Backend classifies and discards them, never persisting user motion data.
- **Canonical Signal** — the standardized form all model input takes: total acceleration (gravity included) in g units, 3 channels (x, y, z), resampled to 50 Hz. Defined by the merge pipeline; the Backend must transform every Recording into this form before inference.
